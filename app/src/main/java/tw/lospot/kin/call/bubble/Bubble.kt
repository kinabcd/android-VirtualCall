...
class Bubble(context: Context, private val call: Call) : Call.Listener {
    ...
    private val marginHorizontal =
        TypedValue.applyDimension(COMPLEX_UNIT_DIP, 8f, context.resources.displayMetrics).toInt()
    private val clickThreshold =
        TypedValue.applyDimension(COMPLEX_UNIT_DIP, 5f, context.resources.displayMetrics).toInt()
    ...
    private fun updateViews() {
        ...
        if (call.isExternal) {
            ...
            idView.backgroundTintList = ColorStateList.valueOf(
                when (call.state) {
        ...
        override fun onTouch(view: View, motionEvent: MotionEvent): Boolean {
            when (motionEvent.actionMasked) {
                ...
                MotionEvent.ACTION_MOVE -> {
                    ...
                    if (maybeClick && sqrt(
                            deltaX.toDouble().pow(2.0) + deltaY.toDouble().pow(2.0)
                        ) > clickThreshold
                    ) {
                        maybeClick = false
                        if (targetState == State.EXPANDED) {
